  ...
  return (
    <header className="sticky top-0 z-30 flex h-14 items-center gap-2 border-b bg-background/80 px-3 backdrop-blur-md sm:px-4">
      ...
      <div className="ml-auto flex items-center gap-1.5 sm:gap-2">
        ...
        <Button variant="outline" size="icon" className="md:hidden" aria-label="Search" onClick={() => setOpenSearch(true)}>
          <Search className="size-4" />
        </Button>
        ...
        <div className="hidden items-center gap-1.5 rounded-md border bg-muted/40 px-2.5 py-1.5 text-xs font-medium lg:flex">
        ...
        <Popover>
          <PopoverTrigger asChild>
            <Button variant="ghost" size="icon" className="relative" aria-label="Notifications">
              ...
              {unread > 0 && (
                <span className="absolute right-1.5 top-1.5 inline-flex h-2 w-2 rounded-full bg-destructive" />
              )}
            </Button>
          </PopoverTrigger>
          <PopoverContent align="end" className="w-80 p-0">
            ...
            <ul className="max-h-80 divide-y overflow-y-auto">
              {notifications.map((n) => (
                <li key={n.id} className="flex gap-3 p-3 hover:bg-muted/60">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-56">
            ...
            <DropdownMenuSeparator />
            <DropdownMenuItem onClick={() => navigate({ to: "/login" })}>Log out</DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      </div>

      <CommandDialog open={openSearch} onOpenChange={setOpenSearch}>
        <CommandInput placeholder="Search routes, users, clients…" />
        <CommandList>
          ...
          <CommandGroup heading="Navigate">
            {Object.entries(ROUTE_LABELS).map(([slug, label]) => (
              <CommandItem
                ...
                onSelect={() => {
                  setOpenSearch(false);
                  const path = slug === "admin" ? "/admin" : slug.startsWith("schedule-") || ["user-attendance","user-schedules","change-logs","attendance-summary","users"].includes(slug)
                    ? `/admin/${slug === "users" ? "users" : slug}`
                    : `/${slug}`;
                  navigate({ to: path });
                }}
              >
